...
1. Automate initial sql script execution - So when the application starts for the first time, the bronze table with raw data is already automatically populated
2. Create a streamlit tab for the ML part - re-training the model and getting predictions
3. Add more features to the Streamlit app - such as displaying the model's performance metrics and graphs
4. Automate creation of the 'gold' table and ML model training once the application is started for the first time
5. Allow filtering and sorting on the streamlit table with all the 300 thousand car records and improve layout